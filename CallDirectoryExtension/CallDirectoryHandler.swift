...
class CallDirectoryHandler: CXCallDirectoryProvider {
    ...
    let ud = UserDefaults.standard//init(suiteName: "group.CSDQAppGroup");
...
//        载入所有黑名单
        let contactList : [String:String] = ud.value(forKey: "BlackList") as! [String:String];

        let allPhoneNumbers: [String] = contactList.keys.sorted()
        for phoneNumber in allPhoneNumbers {
//            号码需加区号
            context.addBlockingEntry(withNextSequentialPhoneNumber: CXCallDirectoryPhoneNumber("86"+phoneNumber)!)
        }
    }
...
//        载入所有通讯录信息
        ...
        let allPhoneNumbers: [String] = contactList.keys.sorted()
        for phoneNumber in allPhoneNumbers {
            context.addIdentificationEntry(withNextSequentialPhoneNumber: CXCallDirectoryPhoneNumber("86"+phoneNumber)!, label: contactList[phoneNumber]!)
        }
    }
